...
> `Coverage: extend`
> **归属：AES Product Design。** 本 Reference 是 AES（深信服下一代端点安全）产线专属组件映射，不作为 Common Design 的通用组件规范。本能力在 IDUX 数字输入基础上扩展 AES 范围提示规则；未登记的其他组件能力由 `prd-design-code` 调用 Common Design 或依据项目已有代码处理。
...
- `encapsulation: false`
- 实现方式：组合 IDUX `IxTooltip` 与 `IxInputNumber`，并接入 ProForm 校验状态。
- `componentId` 是稳定的实现基准，不代表已有 AES 独立组件、封装入口或公共 API。
- Coding 阶段必须按下方完整规则自行组合实现，不得声称直接复用了 AES 前端封装。
...
- Tooltip 不得改变输入框、单位或相邻文案的行高和对齐。
- 输入框与前后文案组成同一逻辑行时，沿用表单规范的行内布局规则。

## 业务输入

- 当前业务必须提供实际 `min`、`max`、单位、字段文案、权限和校验触发时机。
- Tooltip 文案必须根据真实 `min`、`max` 生成，并与输入组件的范围校验保持一致。
- 没有明确范围，或范围已经由相邻文案完整表达时，不生成本能力。
...
- 组合规则无法满足业务时返回 [`../04-patterns/form-management.md`](../04-patterns/form-management.md) 补充，不得虚构已有封装。
