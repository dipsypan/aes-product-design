...
- 组件负责生效资产数量的紧凑展示和打开生效资产详情的入口。
- 当前业务负责传入已按 `asset-scope` Feature 计算或由接口返回的生效数量与业务对象 ID。
- 不同业务模块存在 `AES__APP_LIB/EffectiveTags` 与聚合导出差异；编码阶段必须核验目标分支的真实导出路径，不得凭历史页面猜测。
- 必须核验 Props、Events 和空值行为；封装不足时按上游 Feature 补充，不得修改生效资产语义。
