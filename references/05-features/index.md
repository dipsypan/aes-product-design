...
> **归属：AES Product Design。** 本 Index 是 AES（深信服下一代端点安全）产线专属 Feature 索引，不作为 Common Design 的通用能力索引。本文明确规定的 AES Feature 优先于 Common Design；未覆盖事项由 `prd-design-code` 调用 Common Design 补充。
