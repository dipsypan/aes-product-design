...
entity_card:
  ...
  empty_expression:
    mode: forbidden | match-all
    confirmed: true | false
    display_message: "" # 来自上游的当前对象专属文案
    display:
      placement: title-adjacent
      appearance: secondary-text
      visible_when: confirmed-match-all-and-no-condition-rows
      hide_when: any-condition-row-exists
  condition_editor:
...
对象卡片纵向排列时，对象关系独占相邻两张卡片之间的区域。关系文案、方向标识及其选择控件作为整体，以两张卡片共同的内容宽度为基准水平居中，并在卡片间距内垂直居中。

对象关系不属于任一卡片的标题、条件行或表单标签，不得贴齐卡片左边缘，也不得占用表单标签列。卡片宽度或页面宽度变化时，关系整体仍保持居中；不得依赖固定左边距定位。
...
仅当当前对象已明确允许空条件表示任意匹配且已确认时，按以下规则呈现；不得根据卡片为空自动推断业务语义：

- 说明放在当前卡片标题右侧，与标题同一行、垂直居中，间隔沿用产品既有标题与说明间距。
- 标题保持原有样式；说明使用次级灰色文字、常规字重，无背景、边框或图标，不使用标签、警告条或 Tooltip 替代可见说明。
- `display_message` 必须描述当前对象。例如目的进程为空时显示「不添加条件时则表示任意进程」，不得统一套用含义不清的「任意条件」。对象名称和文案来自上游，不固定为目的进程。
- 无条件行且处于已确认任意匹配状态时显示说明；添加第一条条件后立即隐藏，不替换为「已添加 N 个条件」或其他计数文案。
- 存在条件行但尚未填写完整时，属于待完成配置，不显示任意匹配说明，也不能按任意匹配提交。复杂条件结构中的任一后代条件行都计入显隐判断。
- 删除全部条件和条件组、重新进入已确认任意匹配状态后，恢复标题旁的说明；删除最后一个有效条件前的运行时高风险确认仍按本节原有规则执行。
- 空卡片保留底部添加条件入口，不为占位生成虚假的空条件行或空条件组。已确认以任意匹配初始化的卡片不执行 `simple-flat` 默认生成一行的规则。
- 空间不足时，说明可换至标题下方，不得挤压标题或覆盖操作入口。

以下示意仅说明标题旁说明的显隐，不规定尺寸、字段排列或添加入口的视觉样式；条件关系文案仍由上游及所选编辑模式决定。
...
对象卡片的 `empty_expression` 随 `entity_contract.entity_cards` 返回，显隐转换必须写入 `interaction_contracts` 和 `state_contracts`，不得只保留为静态说明。
