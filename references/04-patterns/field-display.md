...
| 字段场景 | 默认展示方式 | 统一引用 |
| --- | --- | --- |
| 威胁等级 | 实心语义标签 | `AES__APP_LIB/ThreatLevelTag` |
| 事件处置状态 | 图标 + 文本 | `aes-incident/.../event_table/components/dispose_state.vue` |
| 实体已隔离、已拦截等终态 | 终态标签 | `aes-incident/.../06-components/DisposalTag.vue` |
| 资产在线/离线 | 状态点或状态文本 | `AES__APP_LIB/AssetCard` 的 `agentStatusOptions` |
| 启用/禁用 | 状态图标 + 文本；可操作时使用状态下拉 | `app-lib/src/const/status_drop_down_menu.ts`、`app-lib/src/business-comp/status_drop_down_menu/` |
| 普通业务分类 | 中性标签或普通文本 | 项目统一 `IxTag` / 标签组组件 |
| 可跳转字段 | 链接文本 | 复用项目既有链接和跳转组件 |

### 2.1 威胁等级

统一使用公共 `ThreatLevelTag`，引用方式：

```ts
import { ThreatLevelTag } from 'AES__APP_LIB/ThreatLevelTag';
```
...
- 启用/禁用统一使用绿色成功图标 +「启用」、红色禁用图标 +「禁用」；仅展示状态时不增加下拉箭头，可修改时复用状态下拉组件。
- 启用、禁用使用固定文案，不改写成“正常”“关闭”等近义词。
