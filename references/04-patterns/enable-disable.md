...
本 Pattern 处理的是业务对象自身是否启用，不处理按钮、字段或页面控件因权限、前置条件而进入的 disabled 状态；操作可执行性继续读取 `button-eligibility.md`。
...
│  └─ 使用 Radio：「启用 / 禁用」
├─ 已有对象支持单条即时修改
│  └─ 使用可选择目标状态的状态入口，并执行本 Pattern 的即时启禁用变更链路
├─ 当前仅允许查看状态
│  └─ 使用状态图标 + 状态文案，不显示下拉箭头
└─ 对多个已有对象执行批量修改
   └─ 使用工具栏「启用」「禁用」操作，并执行本 Pattern 的批量启禁用变更链路
```
...
- 状态值与其他字段一起提交，不在选择 Radio 后立即发送状态变更请求，也不调用处置状态 Pattern。
...
- 状态资格、二次确认、提交、失败处理和状态回写完整执行本 Pattern 的即时启禁用变更链路。
...
- 可执行范围、相同状态排除、确认数量、提交结果和刷新完整执行本 Pattern 的批量启禁用变更链路。
...
- `single-immediate-change`：`required_patterns` 不增加处置状态 Pattern，`downstream_component_capabilities` 增加 `single-object-enable-disable-selector`；Pattern 不指定具体组件。
- `batch-change`：`required_patterns` 不增加处置状态 Pattern。
...
- 即时单条和批量变更均由本 Pattern 执行启禁用确认与请求流程，没有调用处置状态 Pattern。
